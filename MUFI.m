...
function [logevidence, probability, predictive_mean, predictive_variance,...
    ...
    avg_sigma1, avg_sigma1SQ, avg_sigma2, avg_sigma2SQ]...
...
% Check constraints
if gam1  <= 3/2, disp(num2str(gam1)); disp('gamma1 too small, sigma1SQ does not exist'); end
...
for k1 = 1:length(alpha1_grid)
    for k2 = 1:length(alpha2_grid)
        ...
        logprob(k1,k2) = (-gam1)*log(Phi1) + gammaln(gam1) -1/2*(logdet_custom(K1)-logdet_custom(A1))...
         ...
         logevidence = logPLUS(logevidence, logprob(k1,k2)+log(alpha1_grid(2)-alpha1_grid(1))+log(alpha2_grid(2)- alpha2_grid(1)) ) ;
        

    end
end
logevidence = logPLUS(logevidence, -2*log(2));
...
for k1 = 1:length(alpha1_grid)
    for k2 = 1:length(alpha2_grid)
        ...
        sigma1 = sqrt(Phi1)*exp(gammaln(gam1-1/2)-gammaln(gam1));
        sigma1SQ = Phi1/(2*gam1);
        sigma2 = sqrt(Phi2)*exp(gammaln(gam2-1)-gammaln(gam2-1/2));
